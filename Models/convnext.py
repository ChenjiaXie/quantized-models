...
from .misc import ConvNormActivation1
from .misc import StochasticDepth
import numpy as np
from operations import GELUX
...
class CNBlock(nn.Module):
    def __init__(
        ...
        self.block = nn.Sequential(
            ConvX2d(dim, dim, kernel_size=7, padding=3, groups=dim, bias=True),
            Permute([0, 2, 3, 1]),
            norm_layer(dim),
    ...
    def __init__(
        ...
        **kwargs: Any,
            ...
            if cnf.out_channels is not None:
                ...
                layers.append(
                    nn.Sequential(
                        ...
                        ConvX2d(cnf.input_channels, cnf.out_channels, kernel_size=2, stride=2),
                    )
                )
        ...
        for m in self.modules():
            if isinstance(m, (ConvX2d, nn.Linear)):
...
def _convnext(
    ...
    **kwargs: Any,
    ...
    model = ConvNeXt(args,block_setting, stochastic_depth_prob=stochastic_depth_prob, **kwargs)
    if pretrained:
        if arch not in _MODELS_URLS:
            raise ValueError(f"No checkpoint is available for model type {arch}")
        import torch.utils.model_zoo as model_zoo
        model.load_state_dict(model_zoo.load_url(_MODELS_URLS[arch]))
    return model


def convnext_tiny(args,*, pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ConvNeXt:
    ...
    return _convnext(args,"convnext_tiny", block_setting, stochastic_depth_prob, pretrained, progress, **kwargs)


def convnext_small(args,*, pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ConvNeXt:
    ...
    return _convnext(args,"convnext_small", block_setting, stochastic_depth_prob, pretrained, progress, **kwargs)


def convnext_base(args,*, pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ConvNeXt:
    ...
    return _convnext(args,"convnext_base", block_setting, stochastic_depth_prob, pretrained, progress, **kwargs)


def convnext_large(args,*, pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ConvNeXt:
    ...
    stochastic_depth_prob = kwargs.pop("stochastic_depth_prob", 0.5)
    return _convnext(args,"convnext_large", block_setting, stochastic_depth_prob, pretrained, progress, **kwargs)
