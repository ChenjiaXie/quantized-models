...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, args, cnf: InvertedResidualConfig, norm_layer: Callable[..., nn.Module],
        ...
        if cnf.expanded_channels != cnf.input_channels:
            layers.append(ConvBNActivation(args,cnf.input_channels, cnf.expanded_channels, kernel_size=1,
                                           norm_layer=norm_layer, activation_layer=activation_layer))

        # depthwise
        stride = 1 if cnf.dilation > 1 else cnf.stride
        layers.append(ConvBNActivation(args,cnf.expanded_channels, cnf.expanded_channels, kernel_size=cnf.kernel,
                                       stride=stride, dilation=cnf.dilation, groups=cnf.expanded_channels,
                                       norm_layer=norm_layer, activation_layer=activation_layer))
        if cnf.use_se:
            layers.append(se_layer(args,cnf.expanded_channels))

        # project
        layers.append(ConvBNActivation(args,cnf.expanded_channels, cnf.out_channels, kernel_size=1, norm_layer=norm_layer,
...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        for cnf in inverted_residual_setting:
            layers.append(block(args,cnf, norm_layer))
    ...
    def loadPreTrained(self):
        self.load_state_dict(torch.load('qmodels/mobilenet_v3_large_qnnpack-5bcacf28.pth'))
